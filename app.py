...
def evaluate_and_recommend(client, target_name, results, specialty):
    ...
    2. 엔진별 노출 현황 (ChatGPT, Claude, Gemini 각각 타겟 업체를 추천했는지 여부 분석)
...
if st.button("🚀 통합 AI 포지셔닝 실시간 분석 시작", type="primary"):
    if not api_key.startswith("sk-or-"):
        st.error("좌측 사이드바에 유효한 OpenRouter API Key(sk-or-...로 시작)를 입력해주세요!")
    else:
        # OpenRouter 전용 클라이언트 세팅
        client = OpenAI(
        ...
        try:
            ...
            for idx, r in enumerate(multi_ai_results):
                with st.expander(f"💬 [{r['AI 엔진']}] 답변 원본 보기 (질문: {r['환자 질문']})"):
                    st.write(r['응답 결과'])
